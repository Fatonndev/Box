...
import ru.obvilion.box.Input.Buttons;
	...
	public void onTouch (MotionEvent event, DefaultAndroidInput input) {
		...
		synchronized (input) {
			...
			case MotionEvent.ACTION_POINTER_DOWN:
				...
				if (button != -1) postTouchEvent(input, DefaultAndroidInput.TouchEvent.TOUCH_DOWN, x, y, realPointerIndex, button, timeStamp);
			...
			case MotionEvent.ACTION_OUTSIDE:
				...
				if (button != -1) postTouchEvent(input, DefaultAndroidInput.TouchEvent.TOUCH_UP, x, y, realPointerIndex, button, timeStamp);
				input.touchX[realPointerIndex] = x;
				input.touchY[realPointerIndex] = y;
				input.deltaX[realPointerIndex] = 0;
				input.deltaY[realPointerIndex] = 0;
			...
			case MotionEvent.ACTION_MOVE:
				...
				for (int i = 0; i < pointerCount; i++) {
					...
					button = input.button[realPointerIndex];
					if (button != -1)
						postTouchEvent(input, DefaultAndroidInput.TouchEvent.TOUCH_DRAGGED, x, y, realPointerIndex, button, timeStamp);
					else
						postTouchEvent(input, DefaultAndroidInput.TouchEvent.TOUCH_MOVED, x, y, realPointerIndex, 0, timeStamp);
