...
import ru.obvilion.box.Files;
import ru.obvilion.box.Files.FileType;
import ru.obvilion.box.Graphics.DisplayMode;
import ru.obvilion.box.Graphics.Monitor;
...
public class Lwjgl3ApplicationConfiguration extends Lwjgl3WindowConfiguration {
	...
	public static DisplayMode getDisplayMode(Monitor monitor) {
		...
		return new Lwjgl3Graphics.Lwjgl3DisplayMode(((Lwjgl3Graphics.Lwjgl3Monitor)monitor).monitorHandle, videoMode.width(), videoMode.height(), videoMode.refreshRate(),
				videoMode.redBits() + videoMode.greenBits() + videoMode.blueBits());
	}
	...
	public static DisplayMode[] getDisplayModes(Monitor monitor) {
		...
		for (int i = 0; i < result.length; i++) {
			GLFWVidMode videoMode = videoModes.get(i);
			result[i] = new Lwjgl3Graphics.Lwjgl3DisplayMode(((Lwjgl3Graphics.Lwjgl3Monitor)monitor).monitorHandle, videoMode.width(), videoMode.height(),
					videoMode.refreshRate(), videoMode.redBits() + videoMode.greenBits() + videoMode.blueBits());
		}
		return result;
	}
