...
package ru.obvilion.box.backends.gwt.emu.com.badlogic.gdx.utils.reflect;

import java.lang.annotation.Inherited;

import ru.obvilion.gwtref.client.ReflectionCache;
import ru.obvilion.gwtref.client.Type;
...
public final class ClassReflection {
	...
	static public Constructor[] getConstructors (Class c) {
		ru.obvilion.gwtref.client.Constructor[] constructors = ReflectionCache.getType(c).getConstructors();
	...
	static public Method[] getMethods (Class c) {
		ru.obvilion.gwtref.client.Method[] methods = ReflectionCache.getType(c).getMethods();
	...
	static public Method[] getDeclaredMethods (Class c) {
		ru.obvilion.gwtref.client.Method[] methods = ReflectionCache.getType(c).getDeclaredMethods();
		Method[] result = new Method[methods.length];
		for (int i = 0, j = methods.length; i < j; i++) {
			result[i] = new Method(methods[i]);
		}
	...
	static public Field[] getFields (Class c) {
		ru.obvilion.gwtref.client.Field[] fields = ReflectionCache.getType(c).getFields();
	...
	static public Field[] getDeclaredFields (Class c) {
		ru.obvilion.gwtref.client.Field[] fields = ReflectionCache.getType(c).getDeclaredFields();
		Field[] result = new Field[fields.length];
		for (int i = 0, j = fields.length; i < j; i++) {
			result[i] = new Field(fields[i]);
		}
