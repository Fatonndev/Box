...
import ru.obvilion.box.backends.gwt.widgets.TextInputDialogBox;
import ru.obvilion.box.backends.gwt.widgets.TextInputDialogBox.TextInputDialogListener;
import ru.obvilion.box.utils.IntMap;
import ru.obvilion.box.utils.IntSet;
import ru.obvilion.box.utils.IntSet.IntSetIterator;
	...
	static native void addEventListener (JavaScriptObject target, String name, DefaultGwtInput handler, boolean capture) /*-{
		target
				.addEventListener(
						name,
						function(e) {
							handler.@ru.obvilion.box.backends.gwt.DefaultGwtInput::handleEvent(Lcom/google/gwt/dom/client/NativeEvent;)(e);
						}, capture);
	}-*/;

	private static native float getMouseWheelVelocity (NativeEvent evt) /*-{
		var delta = 0.0;
		var agentInfo = @ru.obvilion.box.backends.gwt.GwtApplication::agentInfo()();
