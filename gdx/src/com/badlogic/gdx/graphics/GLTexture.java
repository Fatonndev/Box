...
public abstract class GLTexture implements Disposable {
	...
	public GLTexture (int glTarget) {
		this(glTarget, Box.gl.glGenTexture ());
	}
	...
	public void bind () {
		Box.gl.glBindTexture(glTarget, glHandle);
	}
	...
	public void bind (int unit) {
		Box.gl.glActiveTexture(GL20.GL_TEXTURE0 + unit);
		Box.gl.glBindTexture(glTarget, glHandle);
	}
	...
	public void unsafeSetWrap (TextureWrap u, TextureWrap v, boolean force) {
		if (u != null && (force || uWrap != u)) {
			Box.gl.glTexParameteri(glTarget, GL20.GL_TEXTURE_WRAP_S, u.getGLEnum());
			uWrap = u;
		}
		if (v != null && (force || vWrap != v)) {
			Box.gl.glTexParameteri(glTarget, GL20.GL_TEXTURE_WRAP_T, v.getGLEnum());
			vWrap = v;
		}
	}
	...
	public void unsafeSetFilter (TextureFilter minFilter, TextureFilter magFilter, boolean force) {
		if (minFilter != null && (force || this.minFilter != minFilter)) {
			Box.gl.glTexParameteri(glTarget, GL20.GL_TEXTURE_MIN_FILTER, minFilter.getGLEnum());
			this.minFilter = minFilter;
		}
		if (magFilter != null && (force || this.magFilter != magFilter)) {
			Box.gl.glTexParameteri(glTarget, GL20.GL_TEXTURE_MAG_FILTER, magFilter.getGLEnum());
	...
	public float unsafeSetAnisotropicFilter (float level, boolean force) {
		...
		Box.gl20.glTexParameterf(GL20.GL_TEXTURE_2D, GL20.GL_TEXTURE_MAX_ANISOTROPY_EXT, level);
	...
	public static float getMaxAnisotropicFilterLevel () {
		...
		if (Box.graphics.supportsExtension("GL_EXT_texture_filter_anisotropic")) {
			FloatBuffer buffer = BufferUtils.newFloatBuffer(16);
			buffer.position(0);
			buffer.limit(buffer.capacity());
			Box.gl20.glGetFloatv(GL20.GL_MAX_TEXTURE_MAX_ANISOTROPY_EXT, buffer);
	...
	protected void delete () {
		if (glHandle != 0) {
			Box.gl.glDeleteTexture (glHandle);
			glHandle = 0;
		}
	}
	...
	public static void uploadImageData (int target, TextureData data, int miplevel) {
		...
		Box.gl.glPixelStorei(GL20.GL_UNPACK_ALIGNMENT, 1);
		if (data.useMipMaps()) {
			MipMapGenerator.generateMipMap(target, pixmap, pixmap.getWidth(), pixmap.getHeight());
		} else {
			Box.gl.glTexImage2D(target, miplevel, pixmap.getGLInternalFormat(), pixmap.getWidth(), pixmap.getHeight(), 0,
