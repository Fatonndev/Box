...
package ru.obvilion.box.utils.compression.lzma;

import java.io.IOException;

import ru.obvilion.box.utils.compression.ICodeProgress;
import ru.obvilion.box.utils.compression.rangecoder.BitTreeEncoder;
...
public class Encoder {
	...
	class LiteralEncoder {
		class Encoder2 {
			...
			public void Init () {
				ru.obvilion.box.utils.compression.rangecoder.Encoder.InitBitModels(m_Encoders);
			}

			public void Encode (ru.obvilion.box.utils.compression.rangecoder.Encoder rangeEncoder, byte symbol) throws IOException {
				int context = 1;
				for (int i = 7; i >= 0; i--) {
					int bit = ((symbol >> i) & 1);
			...
			public void EncodeMatched (ru.obvilion.box.utils.compression.rangecoder.Encoder rangeEncoder, byte matchByte,
			...
			public int GetPrice (boolean matchMode, byte matchByte, byte symbol) {
				...
				if (matchMode) {
					for (; i >= 0; i--) {
						int matchBit = (matchByte >> i) & 1;
						int bit = (symbol >> i) & 1;
						price += ru.obvilion.box.utils.compression.rangecoder.Encoder.GetPrice(m_Encoders[((1 + matchBit) << 8)
							+ context], bit);
						context = (context << 1) | bit;
						if (matchBit != bit) {
							i--;
				...
				for (; i >= 0; i--) {
					int bit = (symbol >> i) & 1;
					price += ru.obvilion.box.utils.compression.rangecoder.Encoder.GetPrice(m_Encoders[context], bit);
					context = (context << 1) | bit;
				}
				return price;
			}
	...
	class LenEncoder {
		...
		public void Init (int numPosStates) {
			ru.obvilion.box.utils.compression.rangecoder.Encoder.InitBitModels(_choice);
		...
		public void Encode (ru.obvilion.box.utils.compression.rangecoder.Encoder rangeEncoder, int symbol, int posState)
			throws IOException {
		...
		public void SetPrices (int posState, int numSymbols, int[] prices, int st) {
			int a0 = ru.obvilion.box.utils.compression.rangecoder.Encoder.GetPrice0(_choice[0]);
			int a1 = ru.obvilion.box.utils.compression.rangecoder.Encoder.GetPrice1(_choice[0]);
			int b0 = a1 + ru.obvilion.box.utils.compression.rangecoder.Encoder.GetPrice0(_choice[1]);
			int b1 = a1 + ru.obvilion.box.utils.compression.rangecoder.Encoder.GetPrice1(_choice[1]);
			int i = 0;
	...
	ru.obvilion.box.utils.compression.lz.BinTree _matchFinder = null;
	ru.obvilion.box.utils.compression.rangecoder.Encoder _rangeEncoder = new ru.obvilion.box.utils.compression.rangecoder.Encoder();
	...
	void Create () {
		if (_matchFinder == null) {
			ru.obvilion.box.utils.compression.lz.BinTree bt = new ru.obvilion.box.utils.compression.lz.BinTree();
			int numHashBytes = 4;
	...
	void Init () {
		...
		ru.obvilion.box.utils.compression.rangecoder.Encoder.InitBitModels(_isMatch);
		ru.obvilion.box.utils.compression.rangecoder.Encoder.InitBitModels(_isRep0Long);
		ru.obvilion.box.utils.compression.rangecoder.Encoder.InitBitModels(_isRep);
		ru.obvilion.box.utils.compression.rangecoder.Encoder.InitBitModels(_isRepG0);
		ru.obvilion.box.utils.compression.rangecoder.Encoder.InitBitModels(_isRepG1);
		ru.obvilion.box.utils.compression.rangecoder.Encoder.InitBitModels(_isRepG2);
		ru.obvilion.box.utils.compression.rangecoder.Encoder.InitBitModels(_posEncoders);
	...
	int GetRepLen1Price (int state, int posState) {
		return ru.obvilion.box.utils.compression.rangecoder.Encoder.GetPrice0(_isRepG0[state])
			+ ru.obvilion.box.utils.compression.rangecoder.Encoder.GetPrice0(_isRep0Long[(state << Base.kNumPosStatesBitsMax)
				+ posState]);
	}
	...
	int GetPureRepPrice (int repIndex, int state, int posState) {
		int price;
		if (repIndex == 0) {
			price = ru.obvilion.box.utils.compression.rangecoder.Encoder.GetPrice0(_isRepG0[state]);
			price += ru.obvilion.box.utils.compression.rangecoder.Encoder
				.GetPrice1(_isRep0Long[(state << Base.kNumPosStatesBitsMax) + posState]);
		} else {
			price = ru.obvilion.box.utils.compression.rangecoder.Encoder.GetPrice1(_isRepG0[state]);
			if (repIndex == 1)
				price += ru.obvilion.box.utils.compression.rangecoder.Encoder.GetPrice0(_isRepG1[state]);
			else {
				price += ru.obvilion.box.utils.compression.rangecoder.Encoder.GetPrice1(_isRepG1[state]);
				price += ru.obvilion.box.utils.compression.rangecoder.Encoder.GetPrice(_isRepG2[state], repIndex - 2);
			}
		}
		return price;
	}
	...
	int GetOptimum (int position) throws IOException {
		...
		_optimum[1].Price = ru.obvilion.box.utils.compression.rangecoder.Encoder
		...
		int matchPrice = ru.obvilion.box.utils.compression.rangecoder.Encoder
			.GetPrice1(_isMatch[(_state << Base.kNumPosStatesBitsMax) + posState]);
		int repMatchPrice = matchPrice + ru.obvilion.box.utils.compression.rangecoder.Encoder.GetPrice1(_isRep[_state]);
		...
		int normalMatchPrice = matchPrice + ru.obvilion.box.utils.compression.rangecoder.Encoder.GetPrice0(_isRep[_state]);
		...
		while (true) {
			...
			int curAnd1Price = curPrice
				+ ru.obvilion.box.utils.compression.rangecoder.Encoder.GetPrice0(_isMatch[(state << Base.kNumPosStatesBitsMax)
					+ posState])
			...
			matchPrice = curPrice
				+ ru.obvilion.box.utils.compression.rangecoder.Encoder.GetPrice1(_isMatch[(state << Base.kNumPosStatesBitsMax)
					+ posState]);
			repMatchPrice = matchPrice + ru.obvilion.box.utils.compression.rangecoder.Encoder.GetPrice1(_isRep[state]);
			...
			if (!nextIsChar && matchByte != currentByte) {
				...
				if (lenTest2 >= 2) {
					...
					int nextRepMatchPrice = curAnd1Price
						+ ru.obvilion.box.utils.compression.rangecoder.Encoder
							.GetPrice1(_isMatch[(state2 << Base.kNumPosStatesBitsMax) + posStateNext])
						+ ru.obvilion.box.utils.compression.rangecoder.Encoder.GetPrice1(_isRep[state2]);
					{
			...
			if (newLen >= startLen) {
				normalMatchPrice = matchPrice + ru.obvilion.box.utils.compression.rangecoder.Encoder.GetPrice0(_isRep[state]);
	...
	void FillDistancesPrices () {
		...
		for (int lenToPosState = 0; lenToPosState < Base.kNumLenToPosStates; lenToPosState++) {
			...
			for (posSlot = Base.kEndPosModelIndex; posSlot < _distTableSize; posSlot++)
				_posSlotPrices[st + posSlot] += ((((posSlot >> 1) - 1) - Base.kNumAlignBits) << ru.obvilion.box.utils.compression.rangecoder.Encoder.kNumBitPriceShiftBits);
