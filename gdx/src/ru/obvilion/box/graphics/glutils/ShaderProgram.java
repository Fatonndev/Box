...
// int infoLogLength = intbuf.get(0);
// if (infoLogLength > 1) {
			log = Box.gl20.glGetProgramInfoLog(program);
// }
			return -1;
		}
...
// }
	...
	public void dispose () {
		...
		if (shaders.get(Box.app) != null) shaders.get(Box.app).removeValue(this, true);
	}
	...
	public static int getNumManagedShaderPrograms () {
		return shaders.get(Box.app).size;
	}
	...
	private void fetchUniforms () {
		params.clear();
		Box.gl20.glGetProgramiv(program, GL20.GL_ACTIVE_UNIFORMS, params);
		...
		for (int i = 0; i < numUniforms; i++) {
			...
			params.put(0, 1);
			type.clear();
			String name = Box.gl20.glGetActiveUniform(program, i, params, type);
			int location = Box.gl20.glGetUniformLocation(program, name);
			uniforms.put(name, location);
	...
	private void fetchAttributes () {
		params.clear();
		Box.gl20.glGetProgramiv(program, GL20.GL_ACTIVE_ATTRIBUTES, params);
		...
		for (int i = 0; i < numAttributes; i++) {
			...
			params.put(0, 1);
			type.clear();
			String name = Box.gl20.glGetActiveAttrib(program, i, params, type);
			int location = Box.gl20.glGetAttribLocation(program, name);
			attributes.put(name, location);
