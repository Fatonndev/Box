...
public class ETC1TextureData implements TextureData {
	...
	public void consumeCustomData (int target) {
		...
		if (!Box.graphics.supportsExtension("GL_OES_compressed_ETC1_RGB8_texture")) {
			...
			Box.gl.glCompressedTexImage2D(target, 0, ETC1.ETC1_RGB8_OES, width, height, 0, data.compressedData.capacity()
				- data.dataOffset, data.compressedData);
			if (useMipMaps()) Box.gl20.glGenerateMipmap(GL20.GL_TEXTURE_2D);
		}
		data.dispose();
		data = null;
		isPrepared = false;
