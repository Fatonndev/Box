...
public class TextureArray extends GLTexture {
	...
	public TextureArray (TextureArrayData data) {
		super(GL30.GL_TEXTURE_2D_ARRAY, Box.gl.glGenTexture());
	...
	private static FileHandle[] getInternalHandles (String... internalPaths) {
		...
		for (int i = 0; i < internalPaths.length; i++) {
			handles[i] = Box.files.internal(internalPaths[i]);
		}
		return handles;
	}
	...
	private void load (TextureArrayData data) {
		...
		bind();
		Box.gl30.glTexImage3D(GL30.GL_TEXTURE_2D_ARRAY, 0, data.getInternalFormat(), data.getWidth(), data.getHeight(), data.getDepth(), 0, data.getInternalFormat(), data.getGLType(), null);
	...
	public static int getNumManagedTextureArrays () {
		return managedTextureArrays.get(Box.app).size;
	}
